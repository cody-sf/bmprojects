...
#define SAMPLING_FREQ 44100.0   // Standard audio sample rate
volatile int AMPLITUDE = 50;   // Conservative middle value
volatile int NOISE = 10;       // Conservative middle value
...
// Umbrella Variables
#define LEDS_PER_STRIP 38 // Number of LEDs per strip
...
static CLEDController &led_controller_1 = FastLED.addLeds<LED_TYPE, 5, COLOR_ORDER>(leds[0], LEDS_PER_STRIP);
static CLEDController &led_controller_2 = FastLED.addLeds<LED_TYPE, 18, COLOR_ORDER>(leds[1], LEDS_PER_STRIP);
static CLEDController &led_controller_3 = FastLED.addLeds<LED_TYPE, 13, COLOR_ORDER>(leds[2], LEDS_PER_STRIP);
static CLEDController &led_controller_4 = FastLED.addLeds<LED_TYPE, 12, COLOR_ORDER>(leds[3], LEDS_PER_STRIP);
static CLEDController &led_controller_5 = FastLED.addLeds<LED_TYPE, 14, COLOR_ORDER>(leds[4], LEDS_PER_STRIP);
static CLEDController &led_controller_6 = FastLED.addLeds<LED_TYPE, 27, COLOR_ORDER>(leds[5], LEDS_PER_STRIP);
...
AvailablePalettes AP_palette = AvailablePalettes::burningrainbow;
...
void featureCallback(BLEDevice central, BLECharacteristic characteristic)
{
  ...
  if (!value.empty())
  {
    ...
    if (feature == 0x07)
    {
      ...
      // light_show.apply_scene_updates(palette_stream); // Commented out - ambiguous call
      return;
    }
...
void setup()
{
  Serial.begin(115200);
  delay(1000);
  Serial.println("=== UMBRELLA V2 STARTING ===");
  ...
  // I2S Microphone Configuration - simplified for stability
  i2s_config_t i2s_config = {
      .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX),
      .sample_rate = 44100,
      .bits_per_sample = I2S_BITS_PER_SAMPLE_16BIT,
      .channel_format = I2S_CHANNEL_FMT_ONLY_LEFT,  // Back to LEFT since L/R=GND
      .communication_format = I2S_COMM_FORMAT_I2S,  // Try standard I2S format
      .intr_alloc_flags = ESP_INTR_FLAG_LEVEL1,
      .dma_buf_count = 4,  // Reduce buffer count
  ...
  if (err != ESP_OK)
  {
    ...
    Serial.println("Continuing without I2S...");
    // Don't halt - continue without I2S for debugging
  } else {
  ...
  Serial.println("=== UMBRELLA V2 SETUP COMPLETE ===");
  Serial.print("Sound sensitive: ");
  Serial.println(soundSensitive ? "YES" : "NO");
...
void handleSoundShow()
{
  ...
  int16_t maxSample = 0;  // Track max sample for debugging
  for (int i = 0; i < SAMPLES; i++)
  {
    int16_t sample;
    size_t bytes_read;
    ...
    if (bytes_read > 0)
    {
      vReal[i] = (double)sample;  // Direct assignment
      if (abs(sample) > maxSample) maxSample = abs(sample);
    }
  }
  
  // Debug output every 500 cycles (less frequent)
  ...
  // Map FFT results to LED strips - improved frequency mapping
  for (int i = 1; i < (SAMPLES / 2); i++)  // Start from 1 to include more low frequencies
  {
    if (vReal[i] > NOISE)
    {
      int bandIndex = map(i, 1, SAMPLES / 2, 0, NUM_STRIPS - 1);
      bandValues[bandIndex] += (int)vReal[i];
    }
  }

  // Update each LED strip with better scaling
  int totalBandEnergy = 0;  // Track total energy for debugging
  for (int strip = 0; strip < NUM_STRIPS; strip++)
  {
    int barHeight = (bandValues[strip] * 2) / AMPLITUDE;  // Multiply by 2 for more sensitivity
    if (barHeight > LEDS_PER_STRIP)
      barHeight = LEDS_PER_STRIP;
    
    totalBandEnergy += bandValues[strip];
  ...
  // Debug FFT output every 500 cycles
  if (debugCount % 500 == 0) {
    Serial.print("FFT: energy=");
    Serial.print(totalBandEnergy);
    Serial.print(", bands=[");
    for (int i = 0; i < NUM_STRIPS; i++) {
...
void loadSettings()
{
  ...
  NOISE = preferences.getInt("noise", 200);
  AMPLITUDE = preferences.getInt("amplitude", 500);
  preferences.end();
}
