...
void uiHeader(const char* title, bool withRail) {
  ...
  // Right side: the light's name, green while the link is up.
  const char* name = app.haveTarget
                         ? (app.deviceName[0] ? app.deviceName : "Light")
                         : "No light";
  tft.setTextDatum(MR_DATUM);
  tft.setTextColor(app.connected ? COL_GREEN : COL_CAPTION, COL_BG);
  tft.setViewport(SCREEN_W - 118, 0, 110, HEADER_H);
  tft.drawString(name, 110, HEADER_H / 2 + 1);
  tft.resetViewport();
...
static void drawSliderRow(int16_t x, int16_t y, int16_t w, const char* label,
                          const char* valueText, int fillNum, int fillDen) {
  ...
  tft.drawString(valueText, x + w, y);
  tft.setTextDatum(TL_DATUM);

  int16_t ty = y + 22;
  tft.fillRoundRect(x, ty, w, 6, 3, COL_CHIP);
  int16_t fill = fillDen > 0 ? (int32_t)w * fillNum / fillDen : 0;
  if (fill < 0) fill = 0;
  if (fill > w) fill = w;
...
void uiSliderRow(int16_t x, int16_t y, int16_t w, const char* label, int value) {
  ...
  drawSliderRow(x, y, w, label, pct, value, 100);
}
...
void uiSliderRowRange(int16_t x, int16_t y, int16_t w, const char* label, int value,
  ...
  drawSliderRow(x, y, w, label, val, value - minV, maxV - minV);
}
...
int uiSliderValueRange(int16_t x, int16_t w, int16_t touchX, int minV, int maxV) {
  int v = minV + ((touchX - x) * (maxV - minV)) / w;
...
void uiPaletteCard(const Rect& r, const uint8_t (*colors)[3], uint8_t count,
  ...
  for (int16_t col = 0; col < r.w; col++) {
    ...
    RGB8 c = gradientColorAt(colors, count, t);
  ...
  tft.setTextFont(2);
  if (tft.textWidth(name) > r.w - 8) tft.setTextFont(1);
  tft.setTextDatum(BC_DATUM);
  int16_t cx = r.x + r.w / 2, by = r.y + r.h - 4;
  tft.setTextColor(rgb565(0x000000));
  tft.drawString(name, cx + 1, by + 1);
  tft.setTextColor(COL_TEXT);
  tft.drawString(name, cx, by);
  tft.setTextDatum(TL_DATUM);
  tft.setTextFont(2);
